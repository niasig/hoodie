...
/**
  ...
  * versa.
  */
object SparkAvroSchemaConverters {

    class IncompatibleSchemaException(msg: String, ex: Throwable = null) extends Exception(msg, ex)

    case class  SchemaType(dataType: DataType, nullable: Boolean)

    def toStructField(name: String, avroSchema: Schema): StructField = {
        val schemaType = toSqlType(avroSchema)
        StructField(name, schemaType.dataType, schemaType.nullable)
    }
    ...
    def convertStructToAvro[T](
        ...
        structType.fields.foreach { field =>
            val newField = fieldsAssembler.name(field.name).`type`() match {
                case f if field.nullable => f.nullable()
                case f => f
            }
